...
out_func_steady <- steady.band(y = sstate, 
                            times = time,
                            func = steadystate, 
                            parms = parameters_c1_60_o, 
                            nspec = 1)
...
microbenchmark(ode.band(sstate, time, steadystate, parms = parameters_c1_60_o, 
                  nspec = 1, names = "steadystate"), 
               steady.band(sstate, time, steadystate, parms = parameters_c1_60_o, 
                  nspec = 1, names = "steadystate")
               )
...
steadystate <- function(t, c_o2, parameters){
  
  flux    <- c(-p_ds*diff(c(p_c_o2_atm, rep(c_o2, p_I-2)))/p_delta_x, 0) 
  d_c_o2 <- -diff(flux)/diff(t)/p_eps + p_S*p_q10/p_eps 
