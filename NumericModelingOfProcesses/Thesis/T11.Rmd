...
data.frame(depth = c(1:45),
           ode = out_func_ode[1001,2:46],
           steady = out_func_steady_1000$y,
           steady_q10 = out_func_steady_q10_1000$y
           ) %>% 
  pivot_longer(2:4, names_to = 'method', values_to = 'o2_vol') %>% 
  ggplot(.) +
  geom_line(aes(x = o2_vol, y = depth, color = method)) +
  scale_y_reverse()
...
out <- ifelse(SteadyState == T
                ,
                steady.band(y = sstate,
                            # time-step for which steady state is wanted
                            times = 1000,
                            func = steadystate,
                            parms = parameters_c1_160,
