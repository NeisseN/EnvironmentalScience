...
pt <- df %>% 
  ggplot(mapping = aes(x = date_time,
                       ...
                       color = temp,
                       fill = temp))
pt + geom_line() + scale_color_gradient2(low = "darkblue",
  mid = "darkgrey",
  high = "darkred",
  midpoint = 0)
...
pth <- dfh_umg %>% 
  ggplot(mapping = aes(x = date,
                       ...
                       color = th,
                       fill = th))
pth + geom_line() + scale_color_gradient2(low = "blue",
  mid = "black",
  high = "red",
  midpoint = 0)
...
Generating date/time vector. 


```{r}
# check out ggplots stat_ecdf() function
